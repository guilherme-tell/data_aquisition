...
volatile uint32_t g_adc_sample = 0.0f;
...
static void Send_UART_JSON_Benchmark(void);
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (g_process_flg)
        {
			...
			// 1. Aquisição
			float32_t x_n = (float32_t)g_adc_sample/4095*3.3 - OFFSET;
...
/* ============================================================
 *  Envio de Benchmark em JSON (Duração Finita)
 * ============================================================ */
static void Send_UART_JSON_Benchmark(void)
{
    ...
    if (g_sample_idx >= N_SAMPLES)
    {
        return; // Encerra a transmissão ao fim do benchmark
    }

    // 2.ROCOF
	float32_t taxa_fps = FS / (float32_t)NPPC;
	phasor.rocof = (phasor.frequencia - phasor.freq_prev) * taxa_fps;
    ...
    int len = snprintf(json_buffer, sizeof(json_buffer),
                           "{\"id\":%lu,\"mag\":%.4f,\"freq\":%.4f,\"rocof\":%.4f,\"phi\":%.4f}\r\n",
                           frame_id++,
                           phasor.magnitude,
                           phasor.frequencia,
                           phasor.rocof,
    ...
    // 4. Transmissão byte a byte
    for (int i = 0; i < len; i++)
    {
    	uart2_write(json_buffer[i] & 0xFF);
    }
}
...
{
	GPIOA->BSRR = GPIO_BSRR_BS8;		/*Define GPIO A para ALTO*/
	TIM2->CR1 	= TIM_CR1_CEN;			/*Habilita a contagem do Timer 2*/


	EXTI->PR = EXTI_PR_PR13;		//Ocorreu requisicao do trigger
}
